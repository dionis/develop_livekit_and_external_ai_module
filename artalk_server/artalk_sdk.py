...
class ARTalkSDKWrapper:
    ...
    def load(self, device: str = "cuda") -> None:
        ...
        * ``ModelLoadStrategy.EXAMPLE_MODELS`` — delegates to ARTalk's own
        ...
        try:
            ...
            # Disable cuDNN benchmark to prevent CUDNN_STATUS_BAD_PARAM_STREAM_MISMATCH in PyTorch 2.0+
    ...
    def _load_from_scratch(self, device: str) -> None:
        ...
        artalk_path_str = str(self.artalk_path)
        if artalk_path_str not in sys.path:
            sys.path.insert(0, artalk_path_str)
        
        # CLEAR sys.modules to prevent conflict
        ...
        # FORCE LOAD the `app` package directly from file to bypass any
        # sys.path shadowing or uvicorn asyncio namespace caching issues.
        app_init_path = os.path.join(artalk_path_str, "app", "__init__.py")
        if os.path.exists(app_init_path):
            spec = importlib.util.spec_from_file_location("app", app_init_path)
            if spec and spec.loader:
                app_module = importlib.util.module_from_spec(spec)
                sys.modules["app"] = app_module
                # Set __path__ so submodules like app.models can be resolved
                app_module.__path__ = [os.path.join(artalk_path_str, "app")]
                try:
                    spec.loader.exec_module(app_module)
                except Exception as e:
                    logger.error(f"Error executing app module: {e}")
        else:
            logger.error(f"CRITICAL: app/__init__.py not found at {app_init_path}")
